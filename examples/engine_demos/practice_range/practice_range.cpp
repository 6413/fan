...
engine->shadow_enable_tile_mode();
    ...
    void draw_gui() {
      if (gloco()->is_toggled(fan::key_t))
      if (auto h = fan::graphics::gui::hud_interactive{"##hud"}) {
        fan::graphics::gui::text("Player Light");
        fan::graphics::gui::color_edit3("PL Color", (fan::vec3*)&pl_col);
