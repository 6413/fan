...
int main() {
  ...
  while(1) {
    ...
    for (uint32_t i = 0; i < count; i++) {
      s.set(&pile.context, pile.ids[ids[i]], &sprite_t::instance_t::position, 
        s.get(&pile.context, pile.ids[ids[i]], &sprite_t::instance_t::position) + suunnat[i] / 10 * pile.window.get_delta_time());
    }
