...
struct player_t {
  ...
  void update() {
    ...
    visual.set_position(fan::vec3(position, floor(position.y / 64) + (0xFAAA - 2) / 2));
...
int main() {
  ...
  tp.open_compiled("examples/games/hotel_puzzle/texturepack.ftp", lp);

  fte_renderer_t renderer;
  renderer.open(&tp);

  //auto compiled_map = renderer.compile("tilemaps/map_game0_0.fte");
  auto compiled_map = renderer.compile("examples/games/hotel_puzzle/newest_lobby.json");
