...
void DefaultShapeVector::break_queue() {
	write(true, true);
}
...
void TextRenderer::render(std::string text, vec2 position, float scale, const Color& color) {
	if (text.empty()) {
		return;
	}
	shader.Use();
...
unsigned int asd(const std::string& str) {
	int count = 0;
