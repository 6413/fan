...
fan_2d::sprite::sprite(const std::vector<unsigned char>& pixels, const vec2& position, const vec2i& size)
	: basic_single_shape(Shader(shader_paths::single_sprite_path_vs, shader_paths::single_sprite_path_fs), position, size) 
{
...
void fan_2d::sprite::reload_image(const std::vector<unsigned char>& pixels, const vec2i& size)
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, size.x, size.y, 0, GL_BGRA, GL_UNSIGNED_BYTE, pixels.data());
	glGenerateMipmap(GL_TEXTURE_2D);
	glBindTexture(GL_TEXTURE_2D, 0);
}
...
fan_2d::image_info fan_2d::sprite::load_image(const std::vector<unsigned char>& pixels, const vec2i& size)
{
	unsigned int texture_id = 0;
...
void fan_gui::text_renderer::set_scale(uint64_t i, f_t scale, vec2 position)
{
	...
	for (int index = 0; index < m_characters[i].size(); index++) {
		...
		default: {
			position.x += text_button::get_character_x_offset(widths[m_characters[i][index]], scale);
		}
		}
	}
...
void fan_gui::text_button::text_button_vector::set_position(uint64_t i, const vec2& position)
{
	...
	set_size(i, len + get_gap_scale(scale) * 2);
	edit_storage(i, m_texts[i], get_position(i) + get_gap_scale(scale), default_text_color, scale);
	upload_stored();
}

void fan_gui::text_button::text_button_vector::set_press_callback(int key, const std::function<void()>& function)
{
	callbacks::key_callback.add(key, true, function);
}
...
bool fan_gui::text_button::text_button_vector::inside(std::uint64_t i)
{
