...
layout(location = 1) out vec4 instance_outline_color;
layout(location = 2) out vec2 texture_coordinate;
layout(location = 3) out vec2 instance_size;
...
void main() {
  ...
  vec3 angle = v3(64u);
  vec2 local = rotate_around(rp * size, rotation_point, angle.z);
  gl_Position = project2(vec3(local + position.xy, position.z));
  instance_color = v4(32u);
  instance_outline_color = v4(48u);
  texture_coordinate = texture_coordinates[id];
  instance_size = size;
}