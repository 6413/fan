...
void fan::window_t::generate_keycode_to_scancode_table() {
  ...
  for (uint16_t i = KbDesc->min_key_code; i < fan::min(KbDesc->max_key_code, max_keycode); ++i) {
    ...
    static constexpr std::pair<const char*, uint16_t> table[] = {
      ...
      {"CAPS", 0x3a}, {"LFSH", 0x2a}, {"RTSH", 0xe036}, {"LCTL", 0x1d}, {"LALT", 0x38}, {"RALT", 0xe033}, {"LWIN", 0xe05b},
      ...
      {"FK09", 0x43}, {"FK10", 0x44}, {"LEFT", 0xe04b}, {"RIGHT", 0xe04d}, {"UP", 0xe048}, {"DOWN", 0xe050}, {"PGUP", 0xe049}, 
      {"PGDN", 0xe051}, {"HOME", 0xe047}, {"END", 0x4f}, {"INS", 0x52}, {"DELE", 0x53}, {"SCLK", 0x46}, {"KP0", 0x52}, {"KP1", 0x4f}, 
    ...
    for (const auto instance : table) {
      ...
      fan::print(string_to_hex(str.c_str()), string_to_hex(instance.first), str.size(), strlen(instance.first));
