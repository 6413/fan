...
#include _FAN_PATH(graphics/vulkan/vk_pipeline.h)
#include _FAN_PATH(graphics/vulkan/vk_descriptor.h)
...
namespace fan {
	...
	public:
		...
		void create_command_buffers() {
			...
			for (int i = 0; i < commandBuffers[0].size(); i++) {
				...
				for (int k = 0; k < std::size(draw_calls); k++) { 
					...
					std::vector<std::pair<void*, std::pair<uint32_t, std::function<void(uint32_t, uint32_t, void*, fan_2d::opengl::shape)>>>> sorted(draw_calls[k].begin(), draw_calls[k].end());

					std::sort(sorted.begin(), sorted.end(), [&](const std::pair<void*, std::pair<uint32_t, std::function<void(uint32_t, uint32_t, void*, fan_2d::opengl::shape)>>>& a, const std::pair<void*, std::pair<uint32_t, std::function<void(uint32_t, uint32_t, void*, fan_2d::opengl::shape)>>>& b) {
					...
					for (int l = sorted.size(); l--; ) {
						...
						if (sorted[l].second.second) {
							sorted[l].second.second(i, l, sorted[l].first, (fan_2d::opengl::shape)k);
						}
					}
				}
		...
		void push_back_draw_call(const fan_2d::opengl::shape* shape, uint32_t shape_count, void* base, const std::function<void(uint32_t i, uint32_t j, void* base, fan_2d::opengl::shape)>& function) {
		...
		bool set_draw_call_order(uint32_t draw_order_id, const fan_2d::opengl::shape* shape, uint32_t shape_count, void* base) {
		...
		std::unordered_map<void*, std::pair<uint32_t, std::function<void(uint32_t i, uint32_t j, void* base, fan_2d::opengl::shape shape)>>> draw_calls[5];
		...
		static inline fan_2d::opengl::shape draw_topology = fan_2d::opengl::shape::triangle;
