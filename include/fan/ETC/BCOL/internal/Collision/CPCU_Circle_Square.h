...
}__ETC_BCOL_PP(CPCU_Circle_Square_t);

bool
__ETC_BCOL_PP(CPCU_Circle_Square_IsThereCollision)
(
  __ETC_BCOL_PP(CPCU_Circle_Square_t *Data)
){
...
(
  ...
  switch(Data->Stage){
    case 0:{
      *op0Y = p1Y + __copysignf(p1Size + Data->StageData.Corner.CircleOffsetY * p0Size, Data->Sp0Y);
      *op0X = p1X + __copysignf(p1Size + Data->StageData.Corner.CircleOffsetX * p0Size, Data->Sp0X);
    ...
    case 1:{
      if(Data->StageData.Side.SPp0X <= p1Size){
        *op0Y = p1Y + __copysignf(p1Size + p0Size, Data->Sp0Y);
        *op0X = p0X;
        *oDirectionY = __copysignf(1, Data->Sp0Y);
        *oDirectionX = 0;
      }
      else if(Data->StageData.Side.SPp0Y <= p1Size){
        *op0Y = p0Y;
        *op0X = p1X + __copysignf(p1Size + p0Size, Data->Sp0X);
        *oDirectionY = 0;
