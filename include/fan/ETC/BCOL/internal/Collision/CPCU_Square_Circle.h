...
}__ETC_BCOL_PP(CPCU_Square_Circle_t);

bool
__ETC_BCOL_PP(CPCU_Square_Circle_IsThereCollision)
(
  __ETC_BCOL_PP(CPCU_Square_Circle_t) *Data
){
...
(
  ...
  if(SPp0X > p0Size && SPp0Y > p0Size){
    f32_t CornerY = SPp0Y - p0Size;
    f32_t CornerX = SPp0X - p0Size;
    ...
    f32_t CombinedSizeY = p0Size + p1Size;
    f32_t CombinedSizeX = p0Size + p1Size;
...
__ETC_BCOL_PP(CPCU_Square_Circle_Solve)(
  ...
  switch(Data->Stage){
    case 0:{
      *op0Y = p1Y + __copysignf(p0Size + Data->StageData.Corner.CircleOffsetY * p1Size, Data->Sp0Y);
      *op0X = p1X + __copysignf(p0Size + Data->StageData.Corner.CircleOffsetX * p1Size, Data->Sp0X);
    ...
    case 1:{
      if(Data->StageData.Side.SPp0X <= p0Size){
        *op0Y = p1Y + __copysignf(p0Size + p1Size, Data->Sp0Y);
        *op0X = p0X;
        *oDirectionY = __copysignf(1, Data->Sp0Y);
        *oDirectionX = 0;
      }
      else if(Data->StageData.Side.SPp0Y <= p0Size){
        *op0Y = p0Y;
        *op0X = p1X + __copysignf(p0Size + p1Size, Data->Sp0X);
        *oDirectionY = 0;
