...
layout(location = 3) in vec4 layout_color;
layout(location = 4) in float layout_font_size;
layout(location = 5) in vec2 layout_rotation_point;
layout(location = 6) in vec2 layout_texture_coordinate[6];

layout(location = 20) in vec2 layout_light_position;
layout(location = 21) in vec4 layout_light_color;
layout(location = 22) in float layout_light_brightness;
layout(location = 23) in float layout_light_angle;

layout(location = 0) out vec2 texture_coordinate;
layout(location = 1) out float font_size;
layout(location = 2) out vec4 text_color;

layout(location = 20) out vec2 light_position;
layout(location = 21) out vec4 light_color;
layout(location = 22) out float light_brightness;
layout(location = 23) out float light_angle;

layout (location = 10) out vec2 f_position;
...
void main() {
	...
	m = translate(m, vec3(layout_position,  0));
...
//	m[3][1] = middle.y;
    ...
    gl_Position = ubo.projection * ubo.view * m * vec4(rectangle_vertices[gl_VertexIndex % 6].x, rectangle_vertices[gl_VertexIndex % 6].y, 0, 1);

	//gl_Position = vec4((1.0 - 2.0 * gl_Position.xy) * 0.2, 0, 1);
	...
	f_position = vec2(vec4(m * vec4(rectangle_vertices[gl_VertexIndex % 6].x, rectangle_vertices[gl_VertexIndex % 6].y, 0, 1)).xy);

	font_size = layout_font_size;

	texture_coordinate = layout_texture_coordinate[gl_VertexIndex];
