...
namespace fan {
	...
	find_in_file(std::string_view path, std::string_view needle) {
			...
			const auto line_start_pos = data.rfind('\n', pos);
			const auto line_start =
					line_start_pos == std::string::npos ? 0 : line_start_pos + 1;

			const auto line_end_pos = data.find('\n', pos);
			const auto line_end =
					line_end_pos == std::string::npos ? data.size() : line_end_pos;

			const auto line_num =
					static_cast<std::size_t>(
							std::count(data.begin(), data.begin() + pos, '\n')
					) + 1;
