...
public:
     ...
     */
    Bin* alloc(const std::string& id, int32_t w1, int32_t h1) {
        if (w1 > wfree_ || h1 > h_) {
            return nullptr;
        }
        int32_t x1 = x_;
...
public:
     ...
     */
    Bin* packOne(const std::string& id, int32_t w, int32_t h) {
        int32_t y = 0;
        int32_t waste = 0;
        struct {
     ...
     */
    Bin* getBin(const std::string& id) {
        std::map<std::string, Bin*>::iterator it = usedbins_.find(id);
...
protected:
     ...
     */
    Bin* allocFreebin(Bin* bin, const std::string& id, int32_t w, int32_t h) {
     ...
     */
    Bin* allocShelf(Shelf& shelf, const std::string& id, int32_t w, int32_t h) {
    ...
    std::map<std::string, Bin*> usedbins_;
    std::vector<Bin*> freebins_;
    std::map<std::string, int32_t> stats_;
};
