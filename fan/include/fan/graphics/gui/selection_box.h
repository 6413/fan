...
#include <fan/graphics/renderer.h>
  ...
  #include <fan/graphics/gui/rectangle_button_sized.h>
...
namespace fan_2d {
  namespace graphics {
    namespace gui {
      ...
      struct selection_box_t {

        typedef void(*checkbox_action_cb)(selection_box_t*, fan::window_t*, uint32_t i);
        ...
        void open(fan::window_t* window, fan::opengl::context_t* context, const open_properties_t& p) {
          ...
          m_check_box.open(window, context);
          m_text.open(context);
          ...
          m_check_box.m_button_event.set_on_input(this, [](fan::window_t* window, fan::opengl::context_t* context, uint32_t i, uint16_t key, fan::key_state key_state, fan_2d::graphics::gui::mouse_stage mouse_stage, void* user_ptr) {
            ...
            ((decltype(this))user_ptr)->m_selection_box_store[i].m_checked = !((decltype(this))user_ptr)->m_selection_box_store[i].m_checked;
            if (((decltype(this))user_ptr)->m_selection_box_store[i].m_checked == 0) {
              ...
              ((decltype(this))user_ptr)->m_check.set_color(context, i * 2, ((decltype(this))user_ptr)->m_selection_box_store[i].m_theme.checkbox.check_color);
              ((decltype(this))user_ptr)->m_check.set_color(context, i * 2 + 1, ((decltype(this))user_ptr)->m_selection_box_store[i].m_theme.checkbox.check_color);
            }
          });
        }
        void close(fan::window_t* window, fan::opengl::context_t* context) {
          m_check.close(context);
          m_check_box.close(window, context);
          m_text.close(context);
        }

        void push_back(fan::window_t* window, fan::opengl::context_t* context, const properties_t& p) {

          f32_t y_offset = m_op.gui_size * (m_check_box.size(window, context) + 1);
          ...
          fan::vec2 bc_position = m_op.position + fan::vec2(0, m_op.gui_size * m_check_box.size(window, context));
          ...
          rp.size = box_size;
          rp.theme = p.theme;
          m_check_box.push_back(window, context, rp);
          ...
          sbs.m_theme = p.theme;
          m_selection_box_store.push_back(sbs);
        }
        ...
        void enable_draw(fan::window_t* window, fan::opengl::context_t* context) {
          ...
          m_check_box.enable_draw(window, context);
     ...
     // protected:
        ...
        fan_2d::graphics::gui::rectangle_button_sized_t m_check_box;
