	...
	namespace physics {
		...
		public:
			...
			void apply_force(uint32_t i, const fan::vec2& force, const fan::vec2& point = 0) {
				m_body[i]->apply_force(force * meter_scale, point);
			}

			void apply_impulse(uint32_t i, const fan::vec2& force, const fan::vec2& point = 0) {
				m_body[i]->apply_impulse(force * meter_scale, point);
			}

			void apply_torque(uint32_t i, f32_t torque) {
				m_body[i]->apply_torque(torque * meter_scale);
			}

			f32_t get_angular_damping(uint32_t i) const {
				return m_body[i]->get_angular_damping() / meter_scale;
			}
			void set_angular_damping(uint32_t i, f32_t amount) {
				m_body[i]->set_angular_damping(amount * meter_scale);
			}

			f32_t get_angular_velocity(uint32_t i) const {
				return m_body[i]->get_angular_velocity() / meter_scale;
			}
			void set_angular_velocity(uint32_t i, f32_t velocity) const {
				m_body[i]->set_angular_velocity(velocity * meter_scale);
			}

			fan_2d::fixture* get_fixture_list(uint32_t i) {
				return (fan_2d::fixture*)m_body[i]->get_fixture_list();
			}

			f32_t get_gravity_scale(uint32_t i) const {
				return m_body[i]->get_gravity_scale() / meter_scale;
			}
			void set_gravity_scale(uint32_t i, f32_t scale) {
				m_body[i]->set_gravity_scale(scale * meter_scale);
			}

			f32_t get_velocity_damping(uint32_t i) const {
				return m_body[i]->get_velocity_damping() / meter_scale;
			}
			void set_velocity_damping(uint32_t i, f32_t amount) {
				m_body[i]->set_velocity_damping(amount * meter_scale);
			}

			fan::vec2 get_velocity(uint32_t i) const {
				return m_body[i]->get_velocity() / meter_scale;
			}
			void set_velocity(uint32_t i, const fan::vec2& velocity) {
				m_body[i]->set_velocity(velocity * meter_scale);
			}

			body_type get_body_type(uint32_t i) const {
				return (body_type)m_body[i]->get_body_type();
			}

			f32_t get_density(uint32_t i) {
				return m_fixture[i]->get_density();
			}
			void set_density(uint32_t i, f32_t density) {
				m_fixture[i]->set_density(density);
			}

			f32_t get_mass(uint32_t i) {
				return m_body[i]->get_mass();
			}

			bool is_bullet(uint32_t i) const {
				return m_body[i]->is_bullet();
			}

			bool is_awake(uint32_t i) const {
				return m_body[i]->is_awake();
			}
			void set_awake(uint32_t i, bool flag) {
				return m_body[i]->set_awake(flag);
			}

			bool is_enabled(uint32_t i) const {
				return m_body[i]->is_enabled();
			}
			void set_enabled(uint32_t i, bool flag) {
				m_body[i]->set_enabled(flag);
			}

			f32_t get_restitution(uint32_t i) const {
				return m_fixture[i]->get_restitution() / meter_scale;
			}
			void set_restitution(uint32_t i, f32_t restitution) {
				m_fixture[i]->set_restitution(restitution * meter_scale);
			}

			void set_position(uint32_t i, const fan::vec2& position) {
				this->m_body[i]->set_transform((position / meter_scale).b2(), this->m_body[i]->get_angle());
				this->m_body[i]->set_awake(true);
			}

			void set_angle(uint32_t i, f64_t rotation) {
				this->m_body[i]->set_transform(this->m_body[i]->get_position(), -rotation);
			}

			void set_angular_rotation(uint32_t i, f64_t w) {
				this->m_body[i]->set_angular_velocity(w);
				this->m_body[i]->set_awake(true);
			}

			void erase(uintptr_t i) {
				m_world->DestroyBody((b2Body*)get_body(i));
		...
		struct rectangle : public physics_base {
			...
			void push_back(const fan::vec2& position, const fan::vec2& size, f32_t angle, body_type body_type_, fan_2d::physics::body_property body_property = { 1, 10, 0.1 }) {
				b2BodyDef body_def;
				...
				b2FixtureUserData user_data;
				user_data.pointer = m_fixture.size() - 1;
		...
		struct convex : public physics_base {

			using physics_base::physics_base;

			void push_back(const fan::vec2& position, fan::vec2* points, uint8_t points_amount, f32_t angle, body_type body_type_, fan_2d::physics::body_property body_property = { 1, 10, 0.1 }) {
				...
				for (int i = 0; i < points_amount; i++) {
					points[i] = points[i] /  meter_scale;
				}

				b2PolygonShape shape;
				shape.Set((const b2Vec2*)points, points_amount);
