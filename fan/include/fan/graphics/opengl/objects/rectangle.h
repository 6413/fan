...
namespace fan_2d {
	namespace graphics {
		...
		struct rectangle {
			...
			static constexpr uint32_t element_byte_size = offset_rotation_vector + sizeof(fan::vec3);
			...
			fan_2d::graphics::rectangle_corners_t get_corners(fan::opengl::context_t* context, uint32_t i) const {
				...
				auto corners = fan_2d::graphics::get_rectangle_corners_no_rotation(position, size);

				f32_t angle = -this->get_angle(context, i);

				fan::vec2 top_left = fan_2d::graphics::get_transformed_point(corners[0] - mid, angle) + mid;
				fan::vec2 top_right = fan_2d::graphics::get_transformed_point(corners[1] - mid, angle) + mid;
				fan::vec2 bottom_left = fan_2d::graphics::get_transformed_point(corners[2] - mid, angle) + mid;
				fan::vec2 bottom_right = fan_2d::graphics::get_transformed_point(corners[3] - mid, angle) + mid;

				return { top_left, top_right, bottom_left, bottom_right };
			}

			const fan::color get_color(fan::opengl::context_t* context, uint32_t i) const {
				return *(fan::color*)m_glsl_buffer.get_instance(i * vertex_count, element_byte_size, offset_color);
			}
			...
			fan::vec2 get_size(fan::opengl::context_t* context, uint32_t i) const {
				return *(fan::vec2*)m_glsl_buffer.get_instance(i * vertex_count, element_byte_size, offset_size);
			}
			...
			fan::vec2 get_rotation_point(fan::opengl::context_t* context, uint32_t i) const {
				return *(fan::vec2*)m_glsl_buffer.get_instance(i * vertex_count, element_byte_size, offset_rotation_point);
			}
			...
			fan::vec3 get_rotation_vector(fan::opengl::context_t* context, uint32_t i) const {
				return *(fan::vec3*)m_glsl_buffer.get_instance(i * vertex_count, element_byte_size, offset_rotation_vector);
			}
