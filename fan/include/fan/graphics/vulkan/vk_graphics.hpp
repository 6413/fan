		...
		public:
			...
			void push_back(const fan::vec2& position, f32_t radius, const fan::color& color) {
				...
				for (int i = 0; i < m_segments; i++) {
					...
					vertice_vector::push_back({ position + fan::vec2(radius * std::cos(theta), radius * std::sin(theta)), 0, 0, 0, color });
				}
			}
			...
			void edit_data(uint32_t i) {
				vertice_vector::instance_buffer->edit_data(i * m_segments, i * m_segments + m_segments);
			}

			void edit_data(uint32_t begin, uint32_t end) {
				vertice_vector::instance_buffer->edit_data(begin * m_segments, (end - begin + 1) * m_segments);
			}
		protected:
