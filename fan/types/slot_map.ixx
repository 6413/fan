...
export namespace fan {
  ...
  struct slot_map_t {
    ...
    struct id_t {
      ...
      constexpr bool is_valid() const { return index != std::numeric_limits<std::uint32_t>::max(); }
      constexpr bool operator==(const id_t&) const = default;
    };

    std::vector<slot_t> slots;
    std::uint32_t free_head = std::numeric_limits<std::uint32_t>::max();
    std::uint32_t active_count = 0;

    template <typename... args_t>
    id_t emplace(args_t&&... args) {
      active_count++;
      if (free_head == std::numeric_limits<std::uint32_t>::max()) {
        std::uint32_t idx = (std::uint32_t)slots.size();
        slots.push_back({0, std::numeric_limits<std::uint32_t>::max(), true, std::optional<T>(std::in_place, std::forward<args_t>(args)...)});
        return {idx, 0};
      }
    ...
    void clear() {
      slots.clear();
      free_head = std::numeric_limits<std::uint32_t>::max();
      active_count = 0;
    }
    ...
    iterator end() { return {this, (std::uint32_t)slots.size()}; }
  };
}