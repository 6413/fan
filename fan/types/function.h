...
#	endif
		...
		template<typename Result, typename... Arguments>
		struct is_valid_function_argument<function_t<Result(Arguments...)>, Result(Arguments...)>
		{
			static const bool value = false;
		};
...
#		endif
	...
	template<typename Result, typename... Arguments>
	class function_t<Result(Arguments...)>
		: public detail::typedeffer<Result, Arguments...>
	{
	public:
		function_t() FUNC_NOEXCEPT
		{
			initialize_empty();
		}
		function_t(std::nullptr_t) FUNC_NOEXCEPT
		{
			initialize_empty();
		}
		function_t(function_t&& other) FUNC_NOEXCEPT
		{
			initialize_empty();
			swap(other);
		}
		...
		template<typename Allocator>
		function_t(std::allocator_arg_t, const Allocator&)
		{
			// ignore the allocator because I don't allocate
			initialize_empty();
		}
		template<typename Allocator>
		function_t(std::allocator_arg_t, const Allocator&, std::nullptr_t)
		{
			// ignore the allocator because I don't allocate
			initialize_empty();
		}
		template<typename Allocator, typename T>
		function_t(std::allocator_arg_t, const Allocator& allocator, T functor,
		...
		template<typename Allocator>
		function_t(std::allocator_arg_t, const Allocator& allocator, const function_t& other)
			: call(other.call)
		{
			typedef typename std::allocator_traits<Allocator>::template rebind_alloc<function_t> MyAllocator;
			...
			{
				detail::manager_type manager_for_function = &detail::get_default_manager<function_t, MyAllocator>();
				if (other.manager_storage.manager == manager_for_function)
				{
					detail::create_manager<function_t, MyAllocator>(manager_storage, MyAllocator(allocator));
		...
		template<typename Allocator>
		function_t(std::allocator_arg_t, const Allocator&, function_t&& other) FUNC_NOEXCEPT
		{
			// ignore the allocator because I don't allocate
			initialize_empty();
			swap(other);
		...
		function_t& operator=(function_t other) FUNC_NOEXCEPT
		{
			swap(other);
			return *this;
		}
		...
		{
			function_t(std::allocator_arg, allocator, functor).swap(*this);
		}

		void swap(function_t& other) FUNC_NOEXCEPT
		{
...
#		endif
	...
	template<typename T>
	bool operator==(std::nullptr_t, const function_t<T>& rhs) FUNC_NOEXCEPT
	{
		return !rhs;
	}
	template<typename T>
	bool operator==(const function_t<T>& lhs, std::nullptr_t) FUNC_NOEXCEPT
	{
		return !lhs;
	}
	template<typename T>
	...
	template<typename T>
	void swap(function_t<T>& lhs, function_t<T>& rhs)
	{
		lhs.swap(rhs);
	}
...
namespace std
{
	...
	struct uses_allocator<fan::function_t<Result(Arguments...)>, Allocator>
		: std::true_type
	{
	};
}
