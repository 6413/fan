...
export namespace fan::graphics::gui::editor {
  struct gizmo_t {
    ...
    template <typename T>
    bool manipulate(T& shape, const fan::vec2& camera_pos, f32_t zoom, const fan::vec2& viewport_center, f32_t snap) {
      bool changed = false;
      fan::vec2 shape_pos = shape.get_position();
      fan::vec2 shape_size = shape.get_size();
      
      auto* draw_list = get_window_draw_list();
      ...
      // Shape dragging
      push_id("shape_drag");
      set_cursor_screen_pos(p_min);
      invisible_button("##shape_drag", p_max - p_min);
      
      if (is_item_active()) {
        if (!is_dragging) {
          is_dragging = true;
          drag_start_pos = shape_pos;
          drag_mouse_start = get_mouse_pos();
        }
        fan::vec2 mouse_world_delta = (get_mouse_pos() - drag_mouse_start) / zoom;
      ...
      for (int i = 0; i < handle_count; ++i) {
        push_id(i);
        fan::vec2 hp = shape_pos + shape_size * handle_dirs[i];
        
        fan::vec2 screen_hp = (hp - camera_pos) * zoom + viewport_center;
        
        set_cursor_screen_pos(screen_hp - handle_size);
        invisible_button("##handle", fan::vec2(handle_size * 2.f));

        bool is_hovered = is_item_hovered();
        bool is_active = is_item_active();
        
        // Draw handle
        fan::color handle_color = is_active ? fan::color(1.f, 0.f, 0.f, 1.f) : (is_hovered ? fan::color(1.f, 0.8f, 0.f, 1.f) : fan::color(1.f, 1.f, 1.f, 1.f));
        ...
        if (is_active) {
          ...
          fan::vec2 mouse_world = (get_mouse_pos() - viewport_center) / zoom + camera_pos;
          fan::vec2 dir = handle_dirs[i];
          
          if (!fan::window::is_key_down(fan::key_left_shift) && snap > 0.0f) {
            mouse_world.x = std::round(mouse_world.x / snap) * snap;
            mouse_world.y = std::round(mouse_world.y / snap) * snap;
          }
