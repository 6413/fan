struct vfi_t {
  ...
  struct shape_t {
    enum shape { always = 0, rectangle = 1 };
  };

  struct shape_properties_always_t { f32_t z; };
  ...
  struct iflags_t {
    ...
    operator uint32_t() const { return ignore_button; }
  };
  ...
  enum class mouse_stage_e { outside, viewport_inside };
  ...
  // generic union-member wrapper — avoids repeating the boilerplate 4 times
  template <typename base_t>
  struct shape_union_member_t {
    shape_union_member_t() = default;
    shape_union_member_t(base_t& b) { std::memcpy(data, &b, sizeof(base_t)); }
    operator base_t& () { return *reinterpret_cast<base_t*>(data); }
    operator const base_t& () const { return *reinterpret_cast<const base_t*>(data); }
    base_t* operator->() { return reinterpret_cast<base_t*>(data); }
    const base_t* operator->() const { return reinterpret_cast<const base_t*>(data); }
    uint8_t data[sizeof(base_t)];
  };
  ...
  struct common_shape_data_t {
    ...
    f32_t depth;
    union {
      shape_union_member_t<shape_data_always_t>    always;
      shape_union_member_t<shape_data_rectangle_t> rectangle;
    } shape;
  };
  ...
  struct common_shape_properties_t {
    ...
    text_cb_t         text_cb         = [](const text_data_t&)         -> int { return 0; };
    struct {
      shape_union_member_t<shape_properties_always_t>    always;
      shape_union_member_t<shape_properties_rectangle_t> rectangle;
    } shape;
    iflags_t flags;
  ...
  using set_rectangle_t = shape_data_rectangle_t;
...
#pragma pack(pop)
  ...
  // helpers to reduce switch repetition
  shape_data_rectangle_t* rect_data(const fan::graphics::shapes::shape_t& in) {
    auto& s = *(ri_t*)in.GetData(g_shapes->shaper);
    if (s.shape_type == shape_t::rectangle) {
      return &(shape_data_rectangle_t&)s.shape_data->shape.rectangle;
    }
    fan::throw_error_impl("vfi: shape is not rectangle");
    return nullptr;
  }

  static fan::vec2 screen_to_world(const fan::vec2& p, fan::graphics::viewport_t viewport, fan::graphics::camera_t camera) {
#if FAN_DEBUG >= fan_debug_high
    if (viewport.iic()) { fan::throw_error_impl("invalid viewport"); }
    if (camera.iic())   { fan::throw_error_impl("invalid camera"); }
#endif
    ...
    fan::vec2 tp = (p - viewport_position) / viewport_size;
    return fan::vec2(r * tp.x - l * tp.x + l, b * tp.y - t * tp.y + t);
  }

  mouse_stage_e viewport_inside(shape_type_t st, common_shape_data_t* data, const fan::vec2& p) {
    switch (st) {
    case shape_t::always:
  ...
  fan::vec2 transform(const fan::vec2& v, shape_type_t st, common_shape_data_t* sd) {
    switch (st) {
    case shape_t::always:
      return v;
    case shape_t::rectangle: {
      fan::vec2 cam_pos = fan::graphics::ctx()->camera_get(fan::graphics::ctx(), sd->shape.rectangle->camera).position;
      return screen_to_world(v, sd->shape.rectangle->viewport, sd->shape.rectangle->camera) + cam_pos;
    }
    default:
  ...
  fan::graphics::shapes::shape_t push_back(const properties_t& p) {
    ...
    case shape_t::rectangle:
      ...
      (shape_data_rectangle_t&)instance.shape_data->shape.rectangle = p.shape.rectangle;
      break;
    }
  ...
  void erase(shaper_t::ShapeID_t& in) {
    ...
    if (fm)                            { focus.mouse.sic(); }
    if (focus.keyboard.NRI == in.NRI)  { focus.keyboard.sic(); }
    if (focus.text.NRI == in.NRI)      { focus.text.sic(); }
    auto data = ((ri_t*)in.GetData(g_shapes->shaper))->shape_data;
    g_shapes->shaper.remove(in);
    delete data;
    if (fm) { feed_mouse_move(get_mouse_position()); }
  }
  ...
  void init_focus_mouse_flag() { focus.method.mouse.flags.ignore_move_focus_check = false; }

  void set_focus_mouse(const shaper_t::ShapeID_t& id)    { focus.mouse.NRI    = id.NRI; init_focus_mouse_flag(); }
  void set_focus_keyboard(const shaper_t::ShapeID_t& id) { focus.keyboard.NRI = id.NRI; }
  void set_focus_text(const shaper_t::ShapeID_t& id)     { focus.text.NRI     = id.NRI; }

  void invalidate_focus_mouse()    { focus.mouse.sic();    focus.method.mouse.flags.ignore_move_focus_check = false; }
  void invalidate_focus_keyboard() { focus.keyboard.sic(); }
  void invalidate_focus_text()     { focus.text.sic(); }

  fan::vec3 get_position(const fan::graphics::shapes::shape_t& in) {
    auto& s = *(ri_t*)in.GetData(g_shapes->shaper);
    if (s.shape_type == shape_t::rectangle) {
      return fan::vec3(s.shape_data->shape.rectangle->position, s.shape_data->depth);
    }
  ...
  void set_position(fan::graphics::shapes::shape_t& in, const fan::vec3& position) {
    auto& s = *(ri_t*)in.GetData(g_shapes->shaper);
    if (s.shape_type == shape_t::rectangle) {
      s.shape_data->shape.rectangle->position = *(fan::vec2*)&position;
      s.shape_data->depth = position.z;
      return;
    }
  ...
  fan::vec2 get_size(const fan::graphics::shapes::shape_t& in) {
    auto& s = *(ri_t*)in.GetData(g_shapes->shaper);
    if (s.shape_type == shape_t::rectangle) { return s.shape_data->shape.rectangle->size; }
    fan::throw_error_impl("invalid get_size for id");
    return 0;
  }
  void set_size(fan::graphics::shapes::shape_t& in, const fan::vec2& size) {
    auto& s = *(ri_t*)in.GetData(g_shapes->shaper);
    if (s.shape_type == shape_t::rectangle) { s.shape_data->shape.rectangle->size = size; return; }
    fan::throw_error_impl("invalid set_size for id");
  }
  void set_angle(fan::graphics::shapes::shape_t& in, const fan::vec3& angle) {
    auto& s = *(ri_t*)in.GetData(g_shapes->shaper);
    if (s.shape_type == shape_t::rectangle) { s.shape_data->shape.rectangle->angle = angle; return; }
    fan::throw_error_impl("invalid set_angle for id");
  }
  void set_rotation_point(fan::graphics::shapes::shape_t& in, const fan::vec2& rotation_point) {
    auto& s = *(ri_t*)in.GetData(g_shapes->shaper);
    if (s.shape_type == shape_t::rectangle) { s.shape_data->shape.rectangle->rotation_point = rotation_point; return; }
    fan::throw_error_impl("invalid set_rotation_point for id");
  }
  ...
  void feed_mouse_move(const fan::vec2& position) {
    ...
    if (!focus.mouse.iic()) {
      ...
      if (bcbfm != focus.mouse.NRI) {
        auto& data2 = *(ri_t*)focus.mouse.GetData(g_shapes->shaper);
        fan::vec2 tp2 = transform(position, data2.shape_type, data2.shape_data);
        mouse_move_data.mouse_stage = viewport_inside(data2.shape_type, data2.shape_data, tp2);
      }
      if (focus.method.mouse.flags.ignore_move_focus_check) { return; }
    }
      ...
      while (KeyTraverse.Loop(g_shapes->shaper)) {
        ...
        do {
          for (int i = 0; i < BlockTraverse.GetAmount(g_shapes->shaper); ++i) {
            ...
            fan::vec2 tp = transform(position, data.shape_type, data.shape_data);
            if (viewport_inside(data.shape_type, data.shape_data, tp) == mouse_stage_e::viewport_inside) {
  ...
  void feed_mouse_button(uint16_t button, int state) {
    ...
    if (mouse_button_data.mouse_stage == mouse_stage_e::outside) {
      if (!focus.method.mouse.flags.ignore_move_focus_check) {
        focus.mouse.sic();
