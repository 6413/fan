...
export namespace fan::graphics::gui {
  ...
  struct settings_menu_t {
    typedef void(*page_function_t)(settings_menu_t*, const fan::vec2&, const fan::vec2&);
    struct page_t {
      bool toggle = false;
      std::string name;
    ...
    static void begin_menu_left(const char* name, const fan::vec2& pos, const fan::vec2& size);
    static void end_menu_left();
    static void menu_graphics_left(settings_menu_t* menu, const fan::vec2& pos, const fan::vec2& size);
    static void menu_graphics_right(settings_menu_t* menu, const fan::vec2& pos, const fan::vec2& size);
    static void menu_audio_left(settings_menu_t* menu, const fan::vec2& pos, const fan::vec2& size);
    static void menu_audio_right(settings_menu_t* menu, const fan::vec2& pos, const fan::vec2& size);
    void query_current_resolution();
    void on_window_resize(const fan::vec2i& new_size);
    void apply_config(bool construct, bool runtime);
    void mark_dirty();
    void update();
    void change_target_fps(int dir);
    ...
    void render_settings_left(const fan::vec2& pos, const fan::vec2& size);
    void render_settings_right(const fan::vec2& pos, const fan::vec2& size, f32_t min_x);
    ...
    static constexpr int wnd_flags =
      fan::graphics::gui::window_flags_no_move |
      fan::graphics::gui::window_flags_no_collapse |
      fan::graphics::gui::window_flags_no_resize |
