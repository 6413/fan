...
tilemap_loader_t::tilemap_loader_t() {
  impl = new map_list_impl_t();
}
...
tilemap_loader_t::id_t tilemap_loader_t::new_map_node() {
  return impl->bll.NewNodeLast().NRI;
}

void tilemap_loader_t::delete_map_node(id_t id) {
  typename decltype(impl->bll)::nr_t nr;
  nr.gint() = id;
  impl->bll.unlrec(nr);
}

tilemap_loader_t::node_t& tilemap_loader_t::get_map_node(id_t id) {
  typename decltype(impl->bll)::nr_t nr;
  nr.gint() = id;
  return impl->bll[nr];
}

bool tilemap_loader_t::is_map_node_invalid(id_t id) const {
  tilemap_loader_t::map_list_impl_t::map_list_NodeReference_t ref;
  ref.NRI = id;
  return ref.iic();
}

void tilemap_loader_t::invalidate_map_node(id_t& id) const {
  tilemap_loader_t::map_list_impl_t::map_list_NodeReference_t ref;
  ref.NRI = id;
  ref.sic();
  id = ref.NRI;
...
tilemap_loader_t::compiled_map_t tilemap_loader_t::compile(const std::string& filename, const std::source_location& callers_path) {
#if defined(FAN_JSON)
  std::ifstream file(fan::io::file::find_relative_path(filename, callers_path), std::ios::binary | std::ios::ate);
  if (!file) { fan::throw_error("Failed to open file: " + filename); }
  ...
  if (!file.read(&buffer[0], size)) { fan::throw_error("Failed to read file: " + filename); }
  file.close();
    ...
    expand_instance(shape_json, [&](const fan::vec3& offs) {
      if (shape_json.contains("mesh_property")) {
        ...
        if (mesh_prop == fte_t::mesh_property_t::physics_shape) {
          ...
          physics_element.position = base_shape.get_position() + offs;
          physics_element.size = base_shape.get_size();
