...
export namespace fan::graphics::gameplay::items {
  ...
  struct registry_t {
    ...
    const item_definition_t* get_definition(std::uint32_t id) const {
      auto it = definitions.find(id);
      return it != definitions.end() ? &it->second : nullptr;
    }

    gameplay::item_t create_item(std::uint32_t id) const {
  ...
  private:
    std::unordered_map<std::uint32_t, item_definition_t> definitions;
  };
  ...
  gameplay::item_t create(std::uint32_t id) {
