...
namespace fan::graphics::culling {
  ...
  static inline void dbg_aabb(const char* tag, std::uint32_t sid, const fan::physics::aabb_t& a) {
  ...
  void static_grid_init(fan::spatial::static_grid_t<shaper_t::ShapeID_t>& grid, const fan::vec2& world_min, const fan::vec2& cell_size, const fan::vec2i& grid_size) {
    ...
    dbg("[static_grid_init]", "world_min", world_min.x, world_min.y, "cell_size", cell_size.x, cell_size.y, "grid_size", grid_size.x, grid_size.y, "cells", (std::uint32_t)grid.cells.size());
  }
  ...
  void dynamic_grid_init(fan::spatial::dynamic_grid_t<shaper_t::ShapeID_t>& grid, const fan::vec2& world_min, const fan::vec2& cell_size, const fan::vec2i& grid_size) {
    ...
    dbg("[dynamic_grid_init]", "world_min", world_min.x, world_min.y, "cell_size", cell_size.x, cell_size.y, "grid_size", grid_size.x, grid_size.y, "cells", (std::uint32_t)grid.cells.size());
  }
  ...
  static inline void ensure_camera_visible_size(per_camera_state_t& cam_state, std::uint32_t nr) {
  ...
  void add_shape(culling_t& culling, shaper_t::ShapeID_t sid, movement_type_t movement) {
    ...
    if (movement == movement_static) {
      dbg("[add_shape]", "static_shapes.size", (std::uint32_t)culling.registry.static_objects.size());
      ...
      std::uint32_t id = (std::uint32_t)culling.dynamic_grid.objects.size() - 1;
  ...
  void cull(culling_t& culling, shaper_t& shaper, const fan::vec2& view_min_in, const fan::vec2& view_max_in, const fan::graphics::camera_t& camera_nr) {
    ...
    std::uint32_t cam_id = camera_nr.NRI;
    ...
    culling.current_total = (std::uint32_t)culling.registry.static_objects.size() + (std::uint32_t)culling.dynamic_grid.objects.size();
    dbg("[cull]", "totals static", (std::uint32_t)culling.registry.static_objects.size(), "dynamic", (std::uint32_t)culling.dynamic_grid.objects.size(), "total", culling.current_total);
    ...
    std::unordered_map<std::uint32_t, std::uint8_t> new_visible;
