...
export namespace fan::graphics {
  ...
  struct light_t : fan::graphics::shapes::shape_t {
    ...
    template <fan::is_not_vector T>
    light_t(T& parent, const fan::vec2& size, const fan::color& color = fan::colors::white, render_view_t* render_view = fan::graphics::ctx().orthographic_render_view) : 
  ...
  struct sprite_t : fan::graphics::shapes::shape_t {
    ...
    sprite_t(const fan::vec3& position, const fan::vec2& size, const fan::graphics::sprite_sheet_t& sprite_sheet, render_view_t* render_view = fan::graphics::ctx().orthographic_render_view);
...
export namespace fan::graphics {
  ...
  struct terrain_t {
    struct tile_t {
      fan::str_view_t sprite;
      f32_t chance;
    };

    terrain_t(fan::vec2 position, fan::vec2 size, f32_t tile_size, std::initializer_list<tile_t> tiles);

    void spawn_tile(fan::str_view_t sprite_name, fan::vec2i pos, f32_t tile_size);
