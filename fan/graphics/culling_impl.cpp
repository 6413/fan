...
namespace fan::graphics::culling {
  ...
  fan::physics::aabb_t get_shape_aabb(shaper_t::ShapeID_t sid) {
    ...
    g_shapes->visit_shape_draw_data(sid.NRI, [&](auto& properties) {
      if constexpr (requires { properties.position; properties.size; }) {
        ...
        size = fan::vec2(
          std::abs(properties.dst.x - properties.src.x),
          std::abs(properties.dst.y - properties.src.y)
        ) * 0.5f;
        has_bounds = true;
        matched = "srcdst";
      }
    ...
    dbg("[AABB]", "sid", sid.NRI, matched,
      "pos", position.x, position.y, "size", size.x, size.y);
    dbg_aabb("         ->", sid.NRI, aabb);
    return aabb;
  }

  void static_grid_init(static_grid_t& grid,
    const fan::vec2& world_min,
    const fan::vec2& cell_size,
    const fan::vec2i& grid_size) {
    ...
    dbg("[static_grid_init]", "world_min", world_min.x, world_min.y,
      "cell_size", cell_size.x, cell_size.y,
      "grid_size", grid_size.x, grid_size.y,
      "cells", (uint32_t)grid.cells.size());
  }

  void dynamic_grid_init(dynamic_grid_t& grid,
    const fan::vec2& world_min,
    const fan::vec2& cell_size,
    const fan::vec2i& grid_size) {
    grid.world_min = world_min;
    ...
    dbg("[dynamic_grid_init]", "world_min", world_min.x, world_min.y,
      "cell_size", cell_size.x, cell_size.y,
      "grid_size", grid_size.x, grid_size.y,
      "cells", (uint32_t)grid.cells.size());
  }
  ...
  void add_static_shape_to_grid(culling_t& culling, shaper_t::ShapeID_t sid) {
    ...
    culling.registry.aabb_cache[nr].min = aabb.min;
    culling.registry.aabb_cache[nr].max = aabb.max;

    auto min_cell = world_to_cell_clamped(aabb.min, culling.static_grid.world_min,
      culling.static_grid.cell_size, culling.static_grid.grid_size);
    auto max_cell = world_to_cell_clamped(aabb.max, culling.static_grid.world_min,
      culling.static_grid.cell_size, culling.static_grid.grid_size);
    ...
    for (auto& [cam_id, cam_state] : culling.camera_states) {
      ...
      dbg("[add_static_shape_to_grid]", "cam", cam_id, "view_dirty=true");
    }
  }
  ...
  void add_shape(culling_t& culling, shaper_t::ShapeID_t sid, movement_type_t movement) {
    ...
    if (nr >= culling.registry.shapeid_to_movement.size()) {
      ...
      dbg("[add_shape]", "resize registries to", (uint32_t)new_size);
    }
    ...
    if (movement == movement_static) {
      ...
      for (auto& [cam_id, cam_state] : culling.camera_states) {
        if (cam_state.cached_view_min != cam_state.cached_view_max) {
          bool in_view = 
            (aabb.max.x >= cam_state.cached_view_min.x) &&
            (aabb.min.x <= cam_state.cached_view_max.x) &&
            (aabb.max.y >= cam_state.cached_view_min.y) &&
            (aabb.min.y <= cam_state.cached_view_max.y);
          ...
          if (in_view) {
            ...
            dbg("[add_shape]", "static in view, pushed to vram");
          }
        }
      }
    }
      ...
      auto cell = world_to_cell_clamped(center, culling.dynamic_grid.world_min,
        culling.dynamic_grid.cell_size, culling.dynamic_grid.grid_size);
      ...
      dbg("[add_shape]", "dynamic id", id, "sid", nr,
        "center", center.x, center.y,
        "cell", cell.x, cell.y, "idx", idx);
      ...
      for (auto& [cam_id, cam_state] : culling.camera_states) {
        if (cam_state.cached_view_min != cam_state.cached_view_max) {
          ...
          if (in_view) {
            ...
            dbg("[add_shape]", "dynamic in view, pushed to vram");
          }
        }
      }
    }
    ...
    for (auto& [cam_id, cam_state] : culling.camera_states) {
      ...
      if (nr >= cam_state.visible.size()) {
        ...
        dbg("[add_shape]", "cam", cam_id, "visible resized", (uint32_t)cam_state.visible.size());
      }
    }
  }
  ...
  void update_dynamic(culling_t& culling, shaper_t::ShapeID_t sid) {
    ...
    if (new_cell != obj.cell) {
      ...
      dbg("[update_dynamic]", "move id", id, "cell", obj.cell, "->", new_cell,
        "center", center.x, center.y);
      obj.cell = new_cell;
    }
    ...
    for (auto& [cam_id, cam_state] : culling.camera_states) {
      cam_state.view_dirty = true;
      dbg("  -> cam", cam_id, "view_dirty=true");
    }
  }
  ...
  void cull(culling_t& culling, shaper_t& , const fan::vec2& view_min_in, const fan::vec2& view_max_in, const fan::graphics::camera_t& camera_nr) {
    ...
    dbg("[cull]", "cam", cam_id,
      "view_min", view_min.x, view_min.y,
      "view_max", view_max.x, view_max.y,
      "view_dirty", cam_state.view_dirty,
      "cached_min", cam_state.cached_view_min.x, cam_state.cached_view_min.y,
      "cached_max", cam_state.cached_view_max.x, cam_state.cached_view_max.y);
    ...
    dbg("[cull]", "totals static", (uint32_t)culling.registry.static_shapes.size(),
      "dynamic", (uint32_t)culling.dynamic_grid.objects.size(),
      "total", culling.current_total);
    ...
    for (int y = prev_min_cell.y; y <= prev_max_cell.y; ++y) {
      ...
      if (y_still_visible) {
        for (int x = prev_min_cell.x; x < min_cell.x; ++x) {
          int idx = y * grid_width + x;
          for (auto sid : cells[idx].shapes) {
            uint32_t nr = sid.NRI;
            if (nr < visible.size() && visible[nr]) {
              dbg("[hide-left]", "cell", x, y, "sid", nr);
              update_shape_vram_if_camera_matches(sid, camera_nr, false);
              visible[nr] = 0;
            }
          }
        }
        for (int x = max_cell.x + 1; x <= prev_max_cell.x; ++x) {
          int idx = y * grid_width + x;
          for (auto sid : cells[idx].shapes) {
            uint32_t nr = sid.NRI;
            if (nr < visible.size() && visible[nr]) {
              dbg("[hide-right]", "cell", x, y, "sid", nr);
              update_shape_vram_if_camera_matches(sid, camera_nr, false);
              visible[nr] = 0;
            }
          }
        ...
        for (int x = prev_min_cell.x; x <= prev_max_cell.x; ++x) {
          int idx = y * grid_width + x;
          for (auto sid : cells[idx].shapes) {
            uint32_t nr = sid.NRI;
            if (nr < visible.size() && visible[nr]) {
    ...
    for (int y = min_cell.y; y <= max_cell.y; ++y) {
      ...
      for (int x = min_cell.x; x <= max_cell.x; ++x) {
        ...
        for (auto sid : cell_shapes) {
          ...
          bool overlaps =
            (aabb.max.x >= view_min.x) &&
            (aabb.min.x <= view_max.x) &&
            (aabb.max.y >= view_min.y) &&
            (aabb.min.y <= view_max.y);

          dbg("[test]", "cell", x, y, "sid", nr,
            "aabb.min", aabb.min.x, aabb.min.y,
            "aabb.max", aabb.max.x, aabb.max.y,
    ...
    for (auto& obj : culling.dynamic_grid.objects) {
      ...
      bool now_visible =
        obj.max.x >= view_min.x && obj.min.x <= view_max.x &&
        obj.max.y >= view_min.y && obj.min.y <= view_max.y;

      dbg("[dyn-test]", "sid", nr,
        "min", obj.min.x, obj.min.y, "max", obj.max.x, obj.max.y,
        "overlaps", now_visible ? "yes" : "no");
    ...
    dbg("[cull] result", "cam", cam_id,
      "visible", culling.current_visible,
      "total", culling.current_total,
      "culled", (culling.current_total >= culling.current_visible ? culling.current_total - culling.current_visible : 0));

    dbg("[cull] cache-set", "cam", cam_id,
      "prev_min", cam_state.cached_view_min.x, cam_state.cached_view_min.y,
      "prev_max", cam_state.cached_view_max.x, cam_state.cached_view_max.y);

    cam_state.cached_view_min = view_min;
    cam_state.cached_view_max = view_max;
    cam_state.view_dirty = false;

    dbg("[cull] cache-now", "cam", cam_id,
      "min", cam_state.cached_view_min.x, cam_state.cached_view_min.y,
      "max", cam_state.cached_view_max.x, cam_state.cached_view_max.y,
      "view_dirty=false");
  }
  ...
  void cull_camera(culling_t& culling, shaper_t& shaper, const fan::graphics::camera_t& camera_nr) {
    ...
    dbg("[cull_camera]", "cam", camera_nr.NRI,
      "pos", pos.x, pos.y,
      "raw", cam.coordinates.left, cam.coordinates.top, "..", cam.coordinates.right, cam.coordinates.bottom,
      "padded view_min", view_min.x, view_min.y,
      "view_max", view_max.x, view_max.y,
      "padding", culling.padding.x, culling.padding.y,
      "zoom", cam.zoom);
