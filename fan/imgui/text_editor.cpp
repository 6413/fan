...
bool TextEditor::GetShowLineNumbers() const {
  return mShowLineNumbers;
}

void TextEditor::SetShowLineNumbers(bool value) {
...
int TextEditor::InsertTextColoredAt(Coordinates& /* inout */ aWhere, const char* aValue, const fan::color& color)
{
  ...
  while (*aValue != '\0')
  {
    ...
    else
    {
      ...
      auto d = UTF8CharLength(*aValue);
      while (d-- > 0 && *aValue != '\0')
        line.insert(line.begin() + cindex++, Glyph(*aValue++, color));
      ++aWhere.mColumn;
    }
...
void TextEditor::Render()
{
  ...
  mTextStart = mShowLineNumbers ? ImGui::GetFont()->CalcTextSizeA(ImGui::GetFontSize(), FLT_MAX, -1.0f, buf, nullptr, nullptr).x + mLeftMargin : mLeftMargin;
...
void TextEditor::InsertTextColored(const std::string& aValue, const fan::color& color)
{
  ...
  totalLines += InsertTextColoredAt(pos, aValue.c_str(), color);
