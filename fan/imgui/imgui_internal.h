...
// Static Asserts
#define IM_STATIC_ASSERT(_COND) static auto CONCAT(v, __COUNTER__) = []{if constexpr(!std::is_void<decltype(_COND)>::value) assert(_COND);return 0;}()
//custom
...
template<typename T> inline T ImLerp(T a, T b, float t)                  { return (T)(a + (b - a) * (T)t); }
template<typename T> inline void ImSwap(T& a, T& b)                      { T tmp = a; a = b; b = tmp; }
template<typename T> inline T ImAddClampOverflow(T a, T b, T mn, T mx)   { if (b < 0 && (a < mn - b)) return mn; if (b > 0 && (a > mx - b)) return mx; return a + b; }
template<typename T> inline T ImSubClampOverflow(T a, T b, T mn, T mx)   { if (b > 0 && (a < mn + b)) return mn; if (b < 0 && (a > mx + b)) return mx; return a - b; }
// - Misc maths helpers
