...
bool    ImGui_ImplOpenGL3_Init(const char* glsl_version)
{
  ...
  if (major == 0 && minor == 0)
  {
    ...
    const char* gl_version = (const char*)GL_CALL(glGetString(GL_VERSION));
...
void    ImGui_ImplOpenGL3_RenderDrawData(ImDrawData* draw_data)
{
  ...
  #endif
  #ifdef IMGUI_IMPL_OPENGL_HAS_POLYGON_MODE
  GLint last_polygon_mode[2]; GL_CALL(glGetIntegerv(GL_POLYGON_MODE, last_polygon_mode));
  #endif
  ...
  {
    GL_CALL(glPolygonMode(GL_FRONT, (GLenum)last_polygon_mode[0]));
    GL_CALL(glPolygonMode(GL_BACK, (GLenum)last_polygon_mode[1]));
  }
  else
  {
    GL_CALL(glPolygonMode(GL_FRONT_AND_BACK, (GLenum)last_polygon_mode[0]));
  }
...
bool    ImGui_ImplOpenGL3_CreateDeviceObjects()
{
  ...
  GLuint vert_handle = GL_CALL(glCreateShader(GL_VERTEX_SHADER));
  ...
  GLuint frag_handle = GL_CALL(glCreateShader(GL_FRAGMENT_SHADER));
