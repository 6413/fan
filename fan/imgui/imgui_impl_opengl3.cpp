...
{
    fan::opengl::GLuint          GlVersion;               // Extracted at runtime using GL_MAJOR_VERSION, GL_MINOR_VERSION queries (e.g. 320 for GL 3.2)
    char            GlslVersionString[32];   // Specified by user or detected based on compile time GL settings.
    bool            GlProfileIsES2;
    bool            GlProfileIsES3;
    bool            GlProfileIsCompat;
    fan::opengl::GLint           GlProfileMask;
    fan::opengl::GLuint          FontTexture;
    fan::opengl::GLuint          ShaderHandle;
    fan::opengl::GLint           AttribLocationTex;       // Uniforms location
    fan::opengl::GLint           AttribLocationProjMtx;
    fan::opengl::GLuint          AttribLocationVtxPos;    // Vertex attributes location
    fan::opengl::GLuint          AttribLocationVtxUV;
    fan::opengl::GLuint          AttribLocationVtxColor;
    unsigned int    VboHandle, ElementsHandle;
    fan::opengl::GLsizeiptr      VertexBufferSize;
    fan::opengl::GLsizeiptr      IndexBufferSize;
    bool            HasClipOrigin;
...
static void ImGui_ImplOpenGL3_SetupRenderState(ImDrawData* draw_data, int fb_width, int fb_height, fan::opengl::GLuint vertex_array_object)
{
    ImGui_ImplOpenGL3_Data* bd = ImGui_ImplOpenGL3_GetBackendData();
    ...
    GL_CALL(glViewport(0, 0, (fan::opengl::GLsizei)fb_width, (fan::opengl::GLsizei)fb_height));
    ...
    GL_CALL(glBindBuffer(fan::opengl::GL_ARRAY_BUFFER, bd->VboHandle));
    GL_CALL(glBindBuffer(fan::opengl::GL_ELEMENT_ARRAY_BUFFER, bd->ElementsHandle));
    GL_CALL(glEnableVertexAttribArray(bd->AttribLocationVtxPos));
    GL_CALL(glEnableVertexAttribArray(bd->AttribLocationVtxUV));
    GL_CALL(glEnableVertexAttribArray(bd->AttribLocationVtxColor));
    GL_CALL(glVertexAttribPointer(bd->AttribLocationVtxPos,   2, fan::opengl::GL_FLOAT, fan::opengl::GL_FALSE, sizeof(ImDrawVert), (fan::opengl::GLvoid*)IM_OFFSETOF(ImDrawVert, pos)));
    GL_CALL(glVertexAttribPointer(bd->AttribLocationVtxUV,    2, fan::opengl::GL_FLOAT, fan::opengl::GL_FALSE, sizeof(ImDrawVert), (fan::opengl::GLvoid*)IM_OFFSETOF(ImDrawVert, uv)));
    GL_CALL(glVertexAttribPointer(bd->AttribLocationVtxColor, 4, fan::opengl::GL_UNSIGNED_BYTE, fan::opengl::GL_TRUE, sizeof(ImDrawVert), (fan::opengl::GLvoid*)IM_OFFSETOF(ImDrawVert, col)));
}
...
void    ImGui_ImplOpenGL3_RenderDrawData(ImDrawData* draw_data)
{
    ...
    fan::opengl::GLuint vertex_array_object = 0;
#ifdef IMGUI_IMPL_OPENGL_USE_VERTEX_ARRAY
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        const fan::opengl::GLsizeiptr vtx_buffer_size = (fan::opengl::GLsizeiptr)cmd_list->VtxBuffer.Size * (int)sizeof(ImDrawVert);
        const fan::opengl::GLsizeiptr idx_buffer_size = (fan::opengl::GLsizeiptr)cmd_list->IdxBuffer.Size * (int)sizeof(ImDrawIdx);
        if (bd->UseBufferSubData)
        {
            if (bd->VertexBufferSize < vtx_buffer_size)
            {
                bd->VertexBufferSize = vtx_buffer_size;
                GL_CALL(glBufferData(fan::opengl::GL_ARRAY_BUFFER, bd->VertexBufferSize, nullptr, fan::opengl::GL_STREAM_DRAW));
            }
            if (bd->IndexBufferSize < idx_buffer_size)
            {
                bd->IndexBufferSize = idx_buffer_size;
                GL_CALL(glBufferData(fan::opengl::GL_ELEMENT_ARRAY_BUFFER, bd->IndexBufferSize, nullptr, fan::opengl::GL_STREAM_DRAW));
            }
            GL_CALL(glBufferSubData(fan::opengl::GL_ARRAY_BUFFER, 0, vtx_buffer_size, (const fan::opengl::GLvoid*)cmd_list->VtxBuffer.Data));
            GL_CALL(glBufferSubData(fan::opengl::GL_ELEMENT_ARRAY_BUFFER, 0, idx_buffer_size, (const fan::opengl::GLvoid*)cmd_list->IdxBuffer.Data));
        }
        else
        {
            GL_CALL(glBufferData(fan::opengl::GL_ARRAY_BUFFER, vtx_buffer_size, (const fan::opengl::GLvoid*)cmd_list->VtxBuffer.Data, fan::opengl::GL_STREAM_DRAW));
            GL_CALL(glBufferData(fan::opengl::GL_ELEMENT_ARRAY_BUFFER, idx_buffer_size, (const fan::opengl::GLvoid*)cmd_list->IdxBuffer.Data, fan::opengl::GL_STREAM_DRAW));
        }
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                // Bind texture, Draw
                GL_CALL(glBindTexture(fan::opengl::GL_TEXTURE_2D, (fan::opengl::GLuint)(intptr_t)pcmd->GetTexID()));
#ifdef IMGUI_IMPL_OPENGL_MAY_HAVE_VTX_OFFSET
                if (bd->GlVersion >= 320)
                    GL_CALL(glDrawElementsBaseVertex(GL_TRIANGLES, (GLsizei)pcmd->ElemCount, sizeof(ImDrawIdx) == 2 ? GL_UNSIGNED_SHORT : GL_UNSIGNED_INT, (void*)(intptr_t)(pcmd->IdxOffset * sizeof(ImDrawIdx)), (GLint)pcmd->VtxOffset));
                else
#endif
                GL_CALL(glDrawElements(fan::opengl::GL_TRIANGLES, (fan::opengl::GLsizei)pcmd->ElemCount, sizeof(ImDrawIdx) == 2 ? fan::opengl::GL_UNSIGNED_SHORT : fan::opengl::GL_UNSIGNED_INT, (void*)(intptr_t)(pcmd->IdxOffset * sizeof(ImDrawIdx))));
            }
        }
    }
...
bool ImGui_ImplOpenGL3_CreateFontsTexture()
{
    ...
    fan::opengl::GLint last_texture;
    GL_CALL(glGetIntegerv(fan::opengl::GL_TEXTURE_BINDING_2D, &last_texture));
    GL_CALL(glGenTextures(1, &bd->FontTexture));
    GL_CALL(glBindTexture(fan::opengl::GL_TEXTURE_2D, bd->FontTexture));
    GL_CALL(glTexParameteri(fan::opengl::GL_TEXTURE_2D, fan::opengl::GL_TEXTURE_MIN_FILTER, fan::opengl::GL_LINEAR));
    GL_CALL(glTexParameteri(fan::opengl::GL_TEXTURE_2D, fan::opengl::GL_TEXTURE_MAG_FILTER, fan::opengl::GL_LINEAR));
#ifdef GL_UNPACK_ROW_LENGTH // Not on WebGL/ES
    GL_CALL(glPixelStorei(GL_UNPACK_ROW_LENGTH, 0));
#endif
    GL_CALL(glTexImage2D(fan::opengl::GL_TEXTURE_2D, 0, fan::opengl::GL_RGBA, width, height, 0, fan::opengl::GL_RGBA, fan::opengl::GL_UNSIGNED_BYTE, pixels));

    // Store our identifier
    io.Fonts->SetTexID((ImTextureID)(intptr_t)bd->FontTexture);

    // Restore state
    GL_CALL(glBindTexture(fan::opengl::GL_TEXTURE_2D, last_texture));
...
static bool CheckShader(fan::opengl::GLuint handle, const char* desc)
{
    ImGui_ImplOpenGL3_Data* bd = ImGui_ImplOpenGL3_GetBackendData();
...
static bool CheckProgram(fan::opengl::GLuint handle, const char* desc)
{
    ImGui_ImplOpenGL3_Data* bd = ImGui_ImplOpenGL3_GetBackendData();
    fan::opengl::GLint status = 0, log_length = 0;
...
bool    ImGui_ImplOpenGL3_CreateDeviceObjects()
{
    ...
    const fan::opengl::GLchar* vertex_shader_glsl_120 =
    ...
    const fan::opengl::GLchar* vertex_shader_glsl_130 =
        "uniform mat4 ProjMtx;\n"
        "in vec2 Position;\n"
        "in vec2 UV;\n"
        "in vec4 Color;\n"
    ...
    const fan::opengl::GLchar* vertex_shader_glsl_300_es =
    ...
    const fan::opengl::GLchar* vertex_shader_glsl_410_core =
        "layout (location = 0) in vec2 Position;\n"
        "layout (location = 1) in vec2 UV;\n"
        "layout (location = 2) in vec4 Color;\n"
        "uniform mat4 ProjMtx;\n"
    ...
    const fan::opengl::GLchar* fragment_shader_glsl_120 =
        "#ifdef GL_ES\n"
    ...
    const fan::opengl::GLchar* fragment_shader_glsl_130 =
        "uniform sampler2D Texture;\n"
        "in vec2 Frag_UV;\n"
        "in vec4 Frag_Color;\n"
        "out vec4 Out_Color;\n"
    ...
    const fan::opengl::GLchar* fragment_shader_glsl_300_es =
        "precision mediump float;\n"
        "uniform sampler2D Texture;\n"
        "in vec2 Frag_UV;\n"
        "in vec4 Frag_Color;\n"
    ...
    const fan::opengl::GLchar* fragment_shader_glsl_410_core =
        "in vec2 Frag_UV;\n"
        "in vec4 Frag_Color;\n"
        "uniform sampler2D Texture;\n"
        "layout (location = 0) out vec4 Out_Color;\n"
    ...
    const fan::opengl::GLchar* vertex_shader = nullptr;
    const fan::opengl::GLchar* fragment_shader = nullptr;
    if (glsl_version < 130)
    {
