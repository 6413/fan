...
export namespace fan {
  namespace graphics {
    ...
    int get_sws_flags(codec_config_t::scaling_quality_e quality) {
      switch (quality) {
      case codec_config_t::SCALING_FASTEST: return SWS_POINT;
      case codec_config_t::SCALING_FAST: return SWS_FAST_BILINEAR;
      case codec_config_t::SCALING_BALANCED: return SWS_BILINEAR;
      case codec_config_t::SCALING_HIGH: return SWS_BICUBIC;
      case codec_config_t::SCALING_HIGHEST: return SWS_LANCZOS;
    ...
    std::uint8_t convert_pixel_format(::AVPixelFormat fmt) {
      ...
      case AV_PIX_FMT_BGRA: return fan::graphics::image_format_e::bgra;
      default: return 0;
    ...
    struct libav_encoder_t {
      ...
      bool update_scaling_quality(codec_config_t::scaling_quality_e new_quality) {
        ...
        encoder->sws_ctx = sws_getContext(
          ...
          fan::graphics::get_sws_flags(new_quality),
          nullptr, nullptr, nullptr
        );

        if (!encoder->sws_ctx) return false;
      ...
      bool init_hardware_acceleration(encoder_state_t* encoder, const codec_config_t& config) {
        ...
        std::vector<AVHWDeviceType> hw_types = {
          ...
          AV_HWDEVICE_TYPE_VAAPI, AV_HWDEVICE_TYPE_VIDEOTOOLBOX
        };
      ...
      void cleanup_encoder_state(encoder_state_t* encoder) {
        ...
        if (encoder->converted_data[0]) av_freep(&encoder->converted_data[0]);
        ...
        if (encoder->hw_device_ctx) av_buffer_unref(&encoder->hw_device_ctx);
      ...
      void maybe_cleanup_old_encoder() {
        ...
        if (now - last_cleanup_time_ < std::chrono::milliseconds(100)) return;
        last_cleanup_time_ = now;

        encoder_state_t* old = pending_cleanup_.exchange(nullptr);
        if (old) cleanup_encoder_state(old);
    ...
    struct libav_decoder_t {
      ...
      static enum AVPixelFormat get_hw_format(AVCodecContext* ctx, const enum AVPixelFormat* pix_fmts) {
        ...
        for (const enum AVPixelFormat* p = pix_fmts; *p != AV_PIX_FMT_NONE; p++) {
          if (*p == decoder->hw_pixel_format_) return *p;
      ...
      bool init_hardware_acceleration() {
        std::vector<std::pair<AVHWDeviceType, const char*>> hw_types = {
          {AV_HWDEVICE_TYPE_CUDA, "cuda"}, {AV_HWDEVICE_TYPE_QSV, "qsv"}, {AV_HWDEVICE_TYPE_VAAPI, "vaapi"},
          {AV_HWDEVICE_TYPE_DXVA2, "dxva2"}, {AV_HWDEVICE_TYPE_D3D11VA, "d3d11va"}, {AV_HWDEVICE_TYPE_VIDEOTOOLBOX, "videotoolbox"}
        };
      ...
      static std::vector<std::string> get_available_hw_decoders() {
        ...
        for (const auto& [codec, decoders] : hw_decoders) {
          for (const auto& decoder : decoders) {
            if (avcodec_find_decoder_by_name(decoder.c_str())) available.push_back(decoder);
    ...
    struct resolution_system_t {
      ...
      static const std::vector<resolution_t>& get_all_resolutions() {
        static const std::vector<resolution_t> resolutions = {
          {3840, 2160, "3840x2160", "4K UHD"}, {2560, 1440, "2560x1440", "QHD/1440p"}, {1920, 1080, "1920x1080", "Full HD/1080p"},
          {1600, 900,  "1600x900",  "HD+"}, {1366, 768,  "1366x768",  "WXGA"}, {1280, 720,  "1280x720",  "HD/720p"},
          {854,  480,  "854x480",   "FWVGA"}, {2560, 1600, "2560x1600", "WQXGA"}, {1920, 1200, "1920x1200", "WUXGA"},

          {1680, 1050, "1680x1050", "WSXGA+"}, {1440, 900,  "1440x900",  "WXGA+"}, {1280, 800,  "1280x800",  "WXGA"},
          {1600, 1200, "1600x1200", "UXGA"}, {1400, 1050, "1400x1050", "SXGA+"}, {1280, 1024, "1280x1024", "SXGA"},
          {1024, 768,  "1024x768",  "XGA"}, {800,  600,  "800x600",   "SVGA"}, {640,  480,  "640x480",   "VGA"},
          {3440, 1440, "3440x1440", "UWQHD"}, {2560, 1080, "2560x1080", "UW-FHD"}, {1920, 800,  "1920x800",  "UW-WXGA"},

          {2048, 1152, "2048x1152", "QWXGA"}, {1856, 1392, "1856x1392", "Custom"}, {1600, 1024, "1600x1024", "Custom 25:16"},
          {1536, 864,  "1536x864",  "Custom 16:9"}, {1344, 756,  "1344x756",  "Custom 16:9"},
        };
      ...
      static std::map<std::string, std::vector<resolution_t>> get_resolutions_by_category() {
        ...
        for (const auto& res : get_all_resolutions()) categorized[res.category].push_back(res);
        return categorized;
      }
      ...
      static std::vector<resolution_t> get_resolutions_by_aspect(f32_t target_aspect, f32_t tolerance = 0.01f) {
        ...
        for (const auto& res : get_all_resolutions()) {
          if (std::abs(res.aspect_ratio() - target_aspect) <= tolerance) filtered.push_back(res);
      ...
      static std::vector<std::string> get_display_strings(bool include_category = false) {
        ...
        if (include_category) {
          for (const auto& [category, resolutions] : get_resolutions_by_category()) {
            for (const auto& res : resolutions) display_strings.push_back(res.name + " (" + category + ")");

          }
        }
        else {
          for (const auto& res : get_all_resolutions()) display_strings.push_back(res.name);
      ...
      static std::vector<resolution_t> get_popular_resolutions() {
        return {
          {3840, 2160, "3840x2160", "4K UHD"}, {2560, 1440, "2560x1440", "QHD"}, {1920, 1080, "1920x1080", "Full HD"},
          {1680, 1050, "1680x1050", "WSXGA+"}, {1600, 900,  "1600x900",  "HD+"}, {1440, 900,  "1440x900",  "WXGA+"},
          {1366, 768,  "1366x768",  "WXGA"}, {1280, 720,  "1280x720",  "HD"}, {1024, 768,  "1024x768",  "XGA"},
      ...
      static resolution_t get_optimal_resolution(std::uint32_t screen_width, std::uint32_t screen_height) {
        ...
        if (!matching_aspect.empty()) {
          for (const auto& res : matching_aspect) {
            if (res.width <= screen_width && res.height <= screen_height) return res;
  ...
  void avcc_to_annexb(const uint8_t* src, size_t src_size, std::vector<uint8_t>& out) {
    out.clear();
  ...
  struct mp4_demuxer_t {
    AVFormatContext* fmt = nullptr;
    int video_stream = -1;
    std::vector<std::uint8_t> sps_pps_;
    ...
    f32_t get_fps() const {
      if (!fmt || video_stream < 0) return 30.f;
      AVRational rate = fmt->streams[video_stream]->avg_frame_rate;
      if (rate.den == 0) {
        rate = fmt->streams[video_stream]->r_frame_rate;
      }
      if (rate.den == 0) return 30.f;
