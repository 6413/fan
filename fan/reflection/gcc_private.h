  ...
  namespace std {
    ...
    namespace meta {
      ...
      class exception : public std::exception {
       ...
       public:
        consteval exception(u8string_view __what,
                            info __from,
        ...
        consteval exception(string_view __what,
                            info __from,
                            source_location __where = source_location::current()) noexcept
            : _M_what{__what}, _M_u8what{_S_exception_cvt_to_utf8(__what)}, _M_from{__from}, _M_where{__where} {}
      ...
      template <class _Rg> concept reflection_range = ranges::input_range<_Rg> &&
      ...
      consteval access_context access_context::via(info __cls) const {
        if (__cls != info{} &&
            (!std::meta::is_type(__cls) || !std::meta::is_class_type(__cls) || !std::meta::is_complete_type(__cls))) {
#if __cpp_exceptions
    ...
    template <ranges::input_range _Rg>
    consteval const ranges::range_value_t<_Rg> *define_static_string(_Rg &&__r) {
      auto __str = meta::reflect_constant_string(__r);
      return meta::extract<const ranges::range_value_t<_Rg> *>(__str);
    }

    namespace __detail {
      template <typename _Rg> concept __statically_sized =
        requires(_Rg &&__r) { static_cast<char (*)[static_cast<size_t>(ranges::size(__r)) >= 0]>(nullptr); };
    }  // namespace __detail

    template <ranges::input_range _Rg>
    consteval auto define_static_array(_Rg &&__r) {
      ...
      if constexpr (__detail::__statically_sized<_Rg>) {
        ...
        if constexpr (__extent)
          return span<const _Tp, __extent>(meta::extract<const _Tp *>(__array), meta::extent(__type, 0U));
        else return span<const _Tp, 0>();
      }
      else {
        if (meta::is_array_type(__type))
          return span<const _Tp>(meta::extract<const _Tp *>(__array), meta::extent(__type, 0U));
        else return span<const _Tp>();
      }
    }

    template <class _Tp>
    consteval const remove_cvref_t<_Tp> *define_static_object(_Tp &&__t) {
      ...
      else return std::define_static_array(span(std::addressof(__t), 1)).data();
    }
